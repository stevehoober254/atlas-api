...
const searchForProperty = asyncHandler(async(req,res)=>{
       const {titleLR} = req.body //req.query  ->lookfor a way to query
    try{
        const user =await getUserById(req.user.id);
        if(!user){
...
//get the first 10 verified properties

const getOnlyFirstVerifiedProperties = asyncHandler(async(req,res)=>{
